...
WITH pipe0510 AS (
  ...
  FROM (
    SELECT DISTINCT run_id, cost_cap_usd FROM apify_capped_runs
  ) d
), down_raw AS (
  ...
  SELECT min(first_success_started_at) AS started_at,
         ...
         count(*) FILTER (WHERE ok_now) AS ok_actions
  FROM down_flags
), step_ok AS (
  SELECT s.action,
  ...
  FROM steps0510 s
  UNION ALL
  SELECT f.action, COALESCE(f.ok_now, false), f.result, f.finished_at
  FROM down_flags f
), portali AS (
  SELECT
