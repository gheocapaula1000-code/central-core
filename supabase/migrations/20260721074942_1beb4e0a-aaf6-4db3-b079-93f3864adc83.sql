...
COMMENT ON FUNCTION public.padova_cluster_points_50m(double precision[], double precision[]) IS
  'Cluster complete-linkage <=50m su liste parallele lat/lng. Nessun chaining transitivo. NULL coords -> cluster -1 (fail-closed a valle).';
...
BEGIN
  ...
  r := public.padova_cluster_points_50m(ARRAY[45.399980, 45.400060]::float8[], ARRAY[11.9, 11.9]::float8[]);
  IF r[1] <> r[2] THEN RAISE EXCEPTION 'FAIL T1: 10 m attraverso vecchio bordo dovrebbero unirsi (r=%)', r; END IF;

  d := public.padova_haversine_m(45.400000, 11.9, 45.400449, 11.9);
  IF d > 50.5 OR d < 49.5 THEN RAISE EXCEPTION 'T2 setup: distanza attesa ~50m, ottenuta % m', d; END IF;
  r := public.padova_cluster_points_50m(ARRAY[45.400000, 45.400449]::float8[], ARRAY[11.9, 11.9]::float8[]);
  IF r[1] <> r[2] THEN RAISE EXCEPTION 'FAIL T2: 50 m dovrebbero unirsi (r=%)', r; END IF;

  d := public.padova_haversine_m(45.400000, 11.9, 45.400460, 11.9);
  IF d <= 50.0 THEN RAISE EXCEPTION 'T3 setup: distanza attesa >50m, ottenuta % m', d; END IF;
  r := public.padova_cluster_points_50m(ARRAY[45.400000, 45.400460]::float8[], ARRAY[11.9, 11.9]::float8[]);
  IF r[1] = r[2] THEN RAISE EXCEPTION 'FAIL T3: 51 m dovrebbero restare distinti (r=%)', r; END IF;

  d := public.padova_haversine_m(45.4139315, 11.8685579, 45.4151999, 11.8675444);
  IF d < 100.0 THEN RAISE EXCEPTION 'T4 setup: distanza attesa >100m, ottenuta % m', d; END IF;
  r := public.padova_cluster_points_50m(ARRAY[45.4139315, 45.4151999]::float8[], ARRAY[11.8685579, 11.8675444]::float8[]);
  IF r[1] = r[2] THEN RAISE EXCEPTION 'FAIL T4: Fusinato/Savonarola distinti (r=%)', r; END IF;

  r := public.padova_cluster_points_50m(ARRAY[45.4, 45.4, 45.4]::float8[], ARRAY[11.9, 11.9005119, 11.9010238]::float8[]);
  IF r[1] <> 1 OR r[2] <> 1 OR r[3] <> 2 THEN RAISE EXCEPTION 'FAIL T5: atteso [1,1,2], ottenuto %', r; END IF;

  r := public.padova_cluster_points_50m(ARRAY[NULL, 45.4]::float8[], ARRAY[NULL, 11.9]::float8[]);
  IF r[1] <> -1 THEN RAISE EXCEPTION 'FAIL T6: NULL coord dovrebbe dare cluster -1 (r=%)', r; END IF;
