...
BEGIN
  ...
  SELECT count(*) INTO v_null_slug_cont FROM public.padova_contendibili WHERE commercial_zone_slug IS NULL;
  SELECT count(*) INTO v_null_slug_mp   FROM public.padova_multi_portale WHERE commercial_zone_slug IS NULL;
  IF v_null_slug_cont > 0 OR v_null_slug_mp > 0 THEN
    RAISE EXCEPTION 'commercial_zone_slug NULL: cont=%, mp=%', v_null_slug_cont, v_null_slug_mp;
  END IF;

  SELECT count(*) INTO v_bad_slug_cont FROM public.padova_contendibili pc
   WHERE NOT EXISTS (SELECT 1 FROM public.civiko_commercial_zones z WHERE z.slug = pc.commercial_zone_slug);
  SELECT count(*) INTO v_bad_slug_mp   FROM public.padova_multi_portale mp
  ...
  IF EXISTS (SELECT 1 FROM public.padova_contendibili GROUP BY chiave_match HAVING count(DISTINCT commercial_zone_slug) > 1) THEN
    RAISE EXCEPTION 'cross-zone group in padova_contendibili';
  END IF;
  IF EXISTS (SELECT 1 FROM public.padova_multi_portale GROUP BY chiave_match HAVING count(DISTINCT commercial_zone_slug) > 1) THEN
  ...
  IF v_id_mismatch_cont > 0 OR v_id_mismatch_mp > 0 THEN
    RAISE EXCEPTION 'ID instabile: cont=%, mp=%', v_id_mismatch_cont, v_id_mismatch_mp;
  END IF;
  ...
  IF v_bad_expired > 0 THEN
    RAISE EXCEPTION 'record expired usati: %', v_bad_expired;
  END IF;
  ...
  IF v_bad_not_padova > 0 THEN
    RAISE EXCEPTION 'record non-Padova usati: %', v_bad_not_padova;
  END IF;
END
$$;
