...
--
-- Non toccare nessun altro endpoint, cron o PWA.
-- Da NON applicare al database remoto in questo intervento.
...
BEGIN
  ...
  END IF;
  IF NOT (v_actual @> (SELECT array_agg(x ORDER BY x) FROM unnest(v_expected) x)
          AND (SELECT array_agg(x ORDER BY x) FROM unnest(v_expected) x) @> v_actual) THEN
...
-- preservando UUID, status e tutti i campi commerciali.
...
-- Verifica anti-collisione (ridondante rispetto alla PK ma esplicita)
DO $mig$
DECLARE v_dup int;
BEGIN
...
AS $fn$
  SELECT btrim(regexp_replace(
    regexp_replace(
      lower(public.unaccent_immutable_or_translate(p_value)),
      '[' || chr(8217) || E'\'`\u00b4.,;:!?()\[\]{}"\u201c\u201d\-_/\\&]+',
      ' ',
      'g'
    ),
...
-- Deno-side normalization non usa l'extension unaccent. Per essere self-contained
-- forniamo un fallback puramente ASCII con translate() sui diacritici latini piu
-- comuni presenti nei quartieri di Padova (a,e,i,o,u,c,n con diacritici).
CREATE OR REPLACE FUNCTION public.unaccent_immutable_or_translate(p_value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $fn$
...
REVOKE ALL ON FUNCTION public.unaccent_immutable_or_translate(text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.unaccent_immutable_or_translate(text) TO service_role;
...
-- Aggiorna zona_slug con il nuovo resolver; elimina le righe non risolvibili
-- (mantieni omi_zone_code sulle righe sicure).
...
DO $mig$
DECLARE v_slugs text := 36992'centro-storico','nord-arcella','est-brenta','est-forcellini-camin','sud-est-sant-osvaldo','sud-voltabarozzo-guizza','sud-ovest-mandria','ovest-chiesanuova-brentelle'36992;
BEGIN
  EXECUTE format(
...
UPDATE public.civiko_commercial_zones z
  ...
  FROM (SELECT slug FROM public.civiko_commercial_zones) all_zones
  LEFT JOIN counts c ON c.slug = all_zones.slug
 WHERE z.slug = all_zones.slug;
...
BEGIN
  ...
  -- Nessun listing con slug diverso dal resolver del proprio quartiere.
  ...
  -- Righe rimaste in quartiere_zona_map coerenti col resolver.
  ...
  -- Ogni riga della nuova mappa deve puntare a uno slug ufficiale.
  ...
  -- Stato e assegnazioni delle 8 righe: preserva status/agency/trial/occupied
  -- rispetto al backup, tenendo conto del rename degli slug legacy.
  FOR v_bak_row IN
  ...
  -- centro-storico: preserva l'eventuale trial/occupazione (id + campi gia' verificati sopra).

  -- padova_listings_zone_v: al massimo uno zone_slug per immobile.
  SELECT COUNT(*) INTO v_bad
