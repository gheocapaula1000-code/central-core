...
-- 1) Backfill comune='Padova' su annunci attivi con quartiere che risolve a zona commerciale ufficiale
UPDATE public.padova_listings
   SET comune = 'Padova'
 WHERE expired_at IS NULL
   AND comune IS NULL
   AND public.civiko_resolve_commercial_zone_slug(quartiere) IS NOT NULL;

-- 2) Redefinizione funzione con regex civico migliorato (unica riga cambiata: estrazione civico_n)
...
BEGIN
  ...
  SELECT
    ...
    -- CIVICO NORMALIZZATO: priorita' pattern Google ", 42, 35xxx ...", fallback trailing
    regexp_replace(
      lower(coalesce(
