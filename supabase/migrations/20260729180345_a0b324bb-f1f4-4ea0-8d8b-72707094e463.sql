...
BEGIN
  ...
  -- MP TOLLERANTE: raggruppa per (czone, via_n, locali) quando >=2 portali diversi
  -- e prezzi entro +-15% (spread max/min <= 1.30), unica agenzia.
  ...
  -- Il testo dell'indirizzo e' portal-specific (casa.it/subito espongono
  -- l'etichetta di quartiere, idealista/immobiliare la via col civico):
  -- usarlo nella chiave comune impedisce di riconoscere lo stesso immobile
  -- pubblicato su portali diversi. Questa branca usa SOLO evidenze robuste:
  -- stessa zona ufficiale, stessi locali, coordinate <= 30 m, mq entro 2%,
  -- prezzo entro 5%, bagni non contraddittori, gruppo max 4 annunci.
  -- Nessun uso di quartiere, via senza civico, fascia di prezzo o testo.
  -- ─────────────────────────────────────────────────────────────────────
  ...
  -- Fail-closed anti-duplicazione: scarta i gruppi geo che riusano URL
  -- gia' presenti nell'output delle branche esistenti.
  ...
  FROM _geo_members m
  JOIN _geo_valid v USING (geo_key)
  WHERE NOT EXISTS (
...
-- ── Recompute di verifica con gate fail-closed (rollback su violazione) ──
DO $gate$
DECLARE
  v_before_cont int; v_before_mp int;
  v_after_cont int;  v_after_mp int;
  v_lost int; v_dup int; v_null int; v_badzone int; v_notpadova int;
  v_res jsonb;
BEGIN
  CREATE TEMP TABLE _pre_cont ON COMMIT DROP AS
    SELECT chiave_match FROM public.padova_contendibili;
  CREATE TEMP TABLE _pre_mp ON COMMIT DROP AS
    SELECT chiave_match FROM public.padova_multi_portale;
  SELECT count(*) INTO v_before_cont FROM _pre_cont;
  SELECT count(*) INTO v_before_mp   FROM _pre_mp;

  v_res := public.recompute_padova_listings_contendibili();

  SELECT count(*) INTO v_after_cont FROM public.padova_contendibili;
  SELECT count(*) INTO v_after_mp   FROM public.padova_multi_portale;
  ...
  IF v_after_cont < v_before_cont THEN
    RAISE EXCEPTION 'GATE regressione contendibili: % -> %', v_before_cont, v_after_cont;
  END IF;
  IF v_after_mp < v_before_mp THEN
    RAISE EXCEPTION 'GATE regressione multi-portale: % -> %', v_before_mp, v_after_mp;
  END IF;

  SELECT count(*) INTO v_lost FROM _pre_cont p
   WHERE NOT EXISTS (SELECT 1 FROM public.padova_contendibili c WHERE c.chiave_match = p.chiave_match);
  RAISE NOTICE 'recompute 5B: % | cont % -> % (persi %) | mp % -> %',
    v_res, v_before_cont, v_after_cont, v_lost, v_before_mp, v_after_mp;
END
$gate$;