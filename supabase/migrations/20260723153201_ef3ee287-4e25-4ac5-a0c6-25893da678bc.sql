...
WITH listings AS (
  SELECT id AS listing_id, url, substring(url from '/annunci/(\d+)') AS pid
  FROM public.padova_listings
  WHERE fonte = 'immobiliare' AND expired_at IS NULL
    AND (commercial_zone_slug IS NULL OR commercial_zone_slug = 'UNRESOLVED')
    AND nullif(trim(coalesce(quartiere, '')), '') IS NULL
),
stag_distinct AS (
  SELECT DISTINCT (raw_json->>'id') AS pid,
         raw_json->'geography'->'microzone'->>'name' AS microzone
  FROM public.padova_immobiliare_detail_staging
  WHERE raw_json ? 'id' AND raw_json->'geography'->'microzone'->>'name' IS NOT NULL
),
stag_unique AS (
  SELECT pid, MAX(microzone) AS microzone FROM stag_distinct
  GROUP BY pid HAVING COUNT(DISTINCT microzone) = 1
)
SELECT l.listing_id, l.url, l.pid, s.microzone
FROM listings l JOIN stag_unique s ON s.pid = l.pid
...
DO $$
DECLARE v_listing_dup int; v_pid_dup int;
BEGIN
  SELECT COUNT(*) INTO v_listing_dup FROM (SELECT listing_id FROM tmp_imm_pairs GROUP BY listing_id HAVING COUNT(*)>1) x;
  IF v_listing_dup > 0 THEN RAISE EXCEPTION 'listing_id collisions: %', v_listing_dup; END IF;
  SELECT COUNT(*) INTO v_pid_dup FROM (SELECT pid FROM tmp_imm_pairs GROUP BY pid HAVING COUNT(*)>1) x;
  IF v_pid_dup > 0 THEN RAISE EXCEPTION 'portal_id collisions: %', v_pid_dup; END IF;
END $$;
...
ALTER TABLE public.padova_listings_immobiliare_quartiere_backfill_20260723 ENABLE ROW LEVEL SECURITY;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='padova_listings_immobiliare_quartiere_backfill_20260723' AND policyname='service_role_all_imm_backfill_20260723') THEN
    CREATE POLICY service_role_all_imm_backfill_20260723 ON public.padova_listings_immobiliare_quartiere_backfill_20260723 TO service_role USING (true) WITH CHECK (true);
  END IF;
END $$;
...
FROM tmp_imm_pairs p JOIN public.padova_listings l ON l.id = p.listing_id
ON CONFLICT (listing_id) DO NOTHING;

DO $$
DECLARE v_total_before int; v_resolved_before int; v_unresolved_before int;
BEGIN
  SELECT COUNT(*),
...
WITH upd AS (
  ...
  WHERE l.id = p.listing_id AND l.fonte = 'immobiliare' AND l.expired_at IS NULL
...
DO $$
DECLARE v_total_after int; v_resolved_after int; v_unresolved_after int; v_out_of_contract int; v_touched_other int;
BEGIN
  SELECT COUNT(*),
         COUNT(*) FILTER (WHERE commercial_zone_slug IS NOT NULL AND commercial_zone_slug <> 'UNRESOLVED'),
         COUNT(*) FILTER (WHERE commercial_zone_slug IS NULL OR commercial_zone_slug = 'UNRESOLVED')
  ...
  FROM public.padova_listings WHERE fonte='immobiliare' AND expired_at IS NULL;
  IF v_total_after <> current_setting('civiko.imm_total_before')::int THEN RAISE EXCEPTION 'total invariance broken: % -> %', current_setting('civiko.imm_total_before'), v_total_after; END IF;
  IF v_resolved_after < current_setting('civiko.imm_resolved_before')::int THEN RAISE EXCEPTION 'resolved decreased'; END IF;
  IF v_unresolved_after > current_setting('civiko.imm_unresolved_before')::int THEN RAISE EXCEPTION 'unresolved increased'; END IF;
  ...
  IF v_out_of_contract > 0 THEN RAISE EXCEPTION 'rows out of contract post-backfill: %', v_out_of_contract; END IF;
  SELECT COUNT(*) INTO v_touched_other
  FROM public.padova_listings_immobiliare_quartiere_backfill_20260723 b
  JOIN public.padova_listings l ON l.id = b.listing_id WHERE l.fonte <> 'immobiliare';
  IF v_touched_other > 0 THEN RAISE EXCEPTION 'other-fonte rows touched: %', v_touched_other; END IF;
END $$;
