...
portali AS (
  ...
  FROM steps0510 s
  WHERE s.ok IS TRUE AND s.finished_at IS NOT NULL AND s.status BETWEEN 200 AND 299
),
classif AS (
  SELECT s.finished_at AS classificazione_ultima,
         (s.ok IS TRUE AND s.status BETWEEN 200 AND 299) AS classificazione_ok
  FROM steps0545 s WHERE s.action = 'private_leads_classify'
),
reco AS (
  SELECT s.finished_at AS recompute_ultimo,
         (s.ok IS TRUE AND s.status BETWEEN 200 AND 299) AS recompute_ok
  FROM steps0545 s WHERE s.action = 'contendibili_recompute'
),
-- L'ack resta legato ESATTAMENTE all'ultimo 0710 concluso prima dello start sync.
ack AS (
...
SELECT
  ...
  -- Ordine di rollout STRETTO sui latest-attempt: nessun overlap ammesso.
  (
