...
DECLARE
  ...
  v_ids           bigint[];
BEGIN
  DROP TABLE IF EXISTS _placeholders;
  ...
  DROP TABLE IF EXISTS _cambio_cand;
  CREATE TEMP TABLE _cambio_cand ON COMMIT DROP AS
  WITH cand AS (
  ...
  v_ids := ARRAY(SELECT cid FROM _cambio_cand);

  IF COALESCE(array_length(v_ids, 1), 0) > 0 THEN
    UPDATE public.padova_contendibili c
       SET cambio_agenzia      = true,
           cambio_agenzia_data = (SELECT k.t_new     FROM _cambio_cand k WHERE k.cid = c.id),
           cambio_agenzia_da   = (SELECT k.name_prev FROM _cambio_cand k WHERE k.cid = c.id),
           cambio_agenzia_a    = (SELECT k.name_new  FROM _cambio_cand k WHERE k.cid = c.id)
...
DECLARE
  ...
  v_ids bigint[];
BEGIN
  DROP TABLE IF EXISTS _zone_avg;
  ...
  DROP TABLE IF EXISTS _first_seen;
  ...
  DROP TABLE IF EXISTS _extras_pk;
  CREATE TEMP TABLE _extras_pk ON COMMIT DROP AS
  SELECT
    c.id AS cid,
    CASE
  ...
  CREATE UNIQUE INDEX ON _extras_pk (cid);

  v_ids := ARRAY(SELECT cid FROM _extras_pk);

  IF COALESCE(array_length(v_ids, 1), 0) > 0 THEN
    UPDATE public.padova_contendibili c
       SET prezzo_immobile_eur_mq   = (SELECT e.prezzo_immobile_eur_mq   FROM _extras_pk e WHERE e.cid = c.id),
           prezzo_medio_zona_eur_mq = (SELECT e.prezzo_medio_zona_eur_mq FROM _extras_pk e WHERE e.cid = c.id),
           differenza_zona_pct      = (SELECT e.differenza_zona_pct      FROM _extras_pk e WHERE e.cid = c.id),
           data_primo_annuncio      = (SELECT e.data_primo_annuncio      FROM _extras_pk e WHERE e.cid = c.id),
           giorni_sul_mercato       = (SELECT e.giorni_sul_mercato       FROM _extras_pk e WHERE e.cid = c.id)
     WHERE c.id = ANY (v_ids);
  END IF;

  DROP TABLE IF EXISTS _signals;
  ...
  DROP TABLE IF EXISTS _press_pk;
  CREATE TEMP TABLE _press_pk ON COMMIT DROP AS
  SELECT
    agg.cid,
    agg.ribasso_pct,
    agg.n_ribassi,
  ...
  CREATE UNIQUE INDEX ON _press_pk (cid);

  v_ids := ARRAY(SELECT cid FROM _press_pk);

  IF COALESCE(array_length(v_ids, 1), 0) > 0 THEN
    UPDATE public.padova_contendibili c
       SET ribasso_pct     = (SELECT p.ribasso_pct     FROM _press_pk p WHERE p.cid = c.id),
           n_ribassi       = (SELECT p.n_ribassi       FROM _press_pk p WHERE p.cid = c.id),
           is_ripubblicato = (SELECT p.is_ripubblicato FROM _press_pk p WHERE p.cid = c.id),
           giorni_fermo    = (SELECT p.giorni_fermo    FROM _press_pk p WHERE p.cid = c.id),
           n_portali       = (SELECT p.n_portali       FROM _press_pk p WHERE p.cid = c.id),
           score_pressione = (SELECT p.score_pressione FROM _press_pk p WHERE p.cid = c.id)
     WHERE c.id = ANY (v_ids);
  END IF;
