...
CREATE OR REPLACE FUNCTION public.norm_agency(p text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  WITH s0 AS (SELECT lower(coalesce(p,'')) AS t),
  s1 AS (SELECT regexp_replace(t, '\s+', ' ', 'g') AS t FROM s0),
  s2 AS (SELECT trim(t) AS t FROM s1),
  -- rimuovi suffissi rete/sede
  s3 AS (SELECT regexp_replace(t, '\s*-\s*padova\s+mls\s*$', '', 'g') AS t FROM s2),
  s4 AS (SELECT regexp_replace(t, '\s+padova\s+mls\s*$', '', 'g') AS t FROM s3),
  s5 AS (SELECT regexp_replace(t, '\s+mls\s*$', '', 'g') AS t FROM s4),
  s6 AS (SELECT regexp_replace(t, '\s*-\s*padova\s*$', '', 'g') AS t FROM s5),
  s7 AS (SELECT regexp_replace(t, '\s+padova\s*$', '', 'g') AS t FROM s6),
  -- rimuovi forme societarie ovunque
  s8 AS (SELECT regexp_replace(t, '\m(s\.?a\.?s\.?|s\.?n\.?c\.?|s\.?r\.?l\.?s?|s\.?p\.?a\.?|& ?c\.?|e c\.?)\M', '', 'g') AS t FROM s7),
  -- pulizia punteggiatura/spazi
  s9 AS (SELECT regexp_replace(t, '[.,;:]+', ' ', 'g') AS t FROM s8),
  s10 AS (SELECT regexp_replace(t, '\s+', ' ', 'g') AS t FROM s9),
  s11 AS (SELECT trim(both ' -' from t) AS t FROM s10)
  SELECT t FROM s11;
$$;
...
BEGIN
  ...
  -- chiave finale: agency_core se non vuota, altrimenti full
  ALTER TABLE _cand ADD COLUMN agency_key text;
  UPDATE _cand SET agency_key = CASE WHEN coalesce(agency_core,'') <> '' THEN agency_core ELSE agency_n_full END;
  ...
  -- per ogni cluster scegli il display name più lungo per ciascuna agency_key
  CREATE TEMP TABLE _agency_display ON COMMIT DROP AS
  SELECT via_n, locali, sub_idx, bagni_key, agency_key,
         (array_agg(agency_raw ORDER BY length(agency_raw) DESC, agency_raw))[1] AS display
  FROM _grp2
  GROUP BY 1,2,3,4,5;
