-- Root-cause fix: recompute contendibili must work with portal-only agency fallback
-- and must not truncate the feed source when zero groups are generated.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      ...
      END, ''),
      'portal:' || coalesce(i.portal, 'unknown')
    ) AS agency_key,
  ...
  UPDATE _cand SET agency_key = 'portal:' || coalesce(portal, 'unknown') WHERE agency_key IS NULL OR agency_key = '';
  ...
  SELECT via_n, locali, tipologia_n, sub_idx, bagni_key, agency_key,
         (array_agg(coalesce(nullif(trim(agency), ''), portal, 'unknown') ORDER BY length(coalesce(nullif(trim(agency), ''), portal, 'unknown')) DESC, coalesce(nullif(trim(agency), ''), portal, 'unknown')))[1] AS display
  FROM _grp2
  ...
  -- Safety: do not wipe the commercial feed source when a scraper/provider
  -- anomaly produces no recomputable groups. Return diagnostics instead.
  IF v_groups > 0 THEN
