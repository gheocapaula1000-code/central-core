...
BEGIN
  ...
  -- Lock deterministici: agenzia + utente, ordine fisso agency -> user.
  PERFORM pg_advisory_xact_lock(
    hashtextextended('civiko_zone_reserve:' || p_agency_id::text, 0)
  );
  PERFORM pg_advisory_xact_lock(
    hashtextextended('civiko_zone_reserve_user:' || p_user_id::text, 0)
  );
  ...
  -- Controllo cross-agency SEMPRE, indipendentemente da v_mem_found.
