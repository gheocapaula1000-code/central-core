...
export interface OMIResult {
  ...
  areaId?: string;
  areaName?: string;
  /** Official AdE quote rows for the resolved link_zona. Never invented. */
  quotes?: OfficialOmiQuote[];
  /** Official Agenzia delle Entrate link_zona (e.g. PD00002850). */
  link_zona?: string;
  /** Semestre of the attached official quotes (e.g. 2025/1). */
  semestre?: string;
  tutteZone?: Array<{
    zona: string;
    zona_descr: string;
...
/**
 * READ-only: every official omi_valori row for one resolved link_zona.
 * Never inserts, updates, or overwrites omi_valori.
 */
...
function buildResult(
  ...
  // Never attach mixed-comune quote rows on a comunale aggregate.
...
export async function lookupOMIByCoordinates(
  ...
  try {
    ...
    const quotes = await fetchOfficialQuotesForLink(supabase, winner.link_zona);
    console.log(`[omi-lookup:coordinates] Multiple polygon matches (${zonesWithPricing.length}), picking narrowest range: zona=${winner.zona} quotes=${quotes.length}`);
    return buildResult(
      winner,
      "polygon_match",
      ...
      comuneStr,
      zoneSummary,
      `Match spaziale: ${zonesWithPricing.length} poligoni candidati, selezionato ${winner.zona} (range più stretto, scelta deterministica)`,
...
export async function lookupOMIByComune(comuneStr: string, codTip = 20): Promise<OMIResult> {
  ...
  if (zonesWithPricing.length === 1) {
    ...
    return buildResult(
      ...
      comune,
      zoneSummary,
      `Zona unica nel comune ${comune}: ${winner.zona}`,
      [],
      quotes,
    );
  }
