...
Deno.serve(async (req) => {
  ...
  if (body.async_start) {
    try {
      ...
      if (mode === "discovery" || mode === "mixed") {
        ...
        const launches = await Promise.all(searchUrls.map(async (surl) => {
          const res = await startApifyRun(
            ACTOR_DISCOVER,
            buildImmobiliareDiscoverInput(surl, desiredResults),
      ...
      if ((mode === "refresh") && refreshUrls.length > 0) {
        ...
        const res = await startApifyRun(
          ACTOR_DETAIL,
          buildImmobiliareDetailInput(refreshUrls, capB),
      ...
      if (base && jobSecret) {
        fetch(`${base}/functions/v1/padova-apify-collect-pending`, {
          method: "POST",
          headers: jobAuthHeaders(jobSecret, anon),
  ...
  try {
    ...
    if ((mode === "refresh" || mode === "mixed" || mode === "discovery") && passBUrls.length > 0) {
      ...
      const { run_id: rid2, dataset_id: did2 } = await startRun(
        ACTOR_DETAIL,
        buildImmobiliareDetailInput(passBUrls, capB),
        token,
      );
      await sb.from("padova_apify_runs").insert({
