...
export function encodeApifyWebhooksParam(webhooks: CollectPendingWebhook[]): string {
  ...
  // base64url-safe standard base64 as required by Apify `webhooks` query param
...
export function classifyNightlyCollectResult(input: NightlySemanticInput): NightlySemanticResult {
  const started = Array.isArray(input.started) ? input.started : [];
...
export function sourceRegistryPatch(
  ...
  if (outcome.ok) {
    ...
    patch.last_error = `[immobiliare-apify] ${String(outcome.error ?? "unknown").slice(0, 450)}`;
  }
  return patch;
}
