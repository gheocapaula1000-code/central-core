...
describe("nightly semantic classification + registry patch", () => {
  ...
  it("prefixes registry errors so F21 last_error is auditable", () => {
    const fail = sourceRegistryPatch({ ok: false, error: "APIFY_START_HTTP_404:actor not found" }, "2026-08-19T02:00:00Z");
...
describe("Padova search URLs and wiring", () => {
  ...
  it("collect-pending accepts Apify webhook bodies via extractCollectRunIds", () => {
...
describe("cron migration — live Core, vault secret, 15-min collect", () => {
  const sql = read("supabase/migrations/20260819170000_immobiliare_apify_collect_handoff.sql");
  const health = read("supabase/functions/core-cron-health-public/index.ts");

  it("exists and targets live Core only", () => {
    expect(existsSync(resolve(root, "supabase/migrations/20260819170000_immobiliare_apify_collect_handoff.sql"))).toBe(true);
  ...
  it("reschedules collect-pending every 15 minutes", () => {
