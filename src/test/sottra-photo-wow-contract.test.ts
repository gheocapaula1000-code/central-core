...
describe("Sottra photoWow — official OMI contract", () => {
  ...
  it("publishes every official omi_valori row as quotes after link_zona is resolved", () => {
    expect(omiLookup).toContain("mapValoriRowsToQuotes");
    expect(omiLookup).toContain("fetchOfficialQuotesForLink");
    expect(scan).toContain("quotes: omi.quotes");
    expect(photoWow).toContain("pricing.quotes");
  });
});
