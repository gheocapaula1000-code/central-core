...
| **sottra** | `false` | `/health`, `/manifest` | All `scan/*`, `forecast/*` routes | origin-policy, app-secret, input-val, envelope, identity | PWA proxies (Sottra, KeyDraft via gateway) | 19 POST routes (incl. photo-wow aliases) |
