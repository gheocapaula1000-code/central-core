...
| Endpoint | Auth | Notes |
|----------|------|--------|
| `core-cron-health-public` | `x-diagnostic-secret` (`DIAGNOSTIC_SECRET`) | **401 without the secret is expected** (Checkpoint 1A). Not anonymously public. Returns cron job status plus `fonti_scheduler` (`last_error`, stale, never-run). |
| `connector-status` | Admin Bearer JWT **or** `x-job-secret` **or** `x-diagnostic-secret` | 401 without one of those is expected. Surfaces `last_error`, `failed_sources`, and trigger coverage. |
