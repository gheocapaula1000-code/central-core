-- 20260721050000_padova_contendibili_identity_gate.sql
--
-- Purpose (CAUSE-FIX, non un altro filtro di prezzo):
--   Correggere la logica che genera `padova_contendibili` in modo che
--   l'identità dell'immobile sia verificata PRIMA di agenzie, prezzo,
--   ribasso e ranking. Prima di questa migration due annunci con la
--   stessa via normalizzata (`arnaldo-fusinato`) ma senza civico e a
--   ~150 m di distanza (Via Fusinato civ. ~5 vs civ. ~12) venivano
--   fusi nello stesso gruppo solo perché condividevano `mq` e `locali`.
--
-- Regola nuova (identity gate, fail-closed):
--   Chiave identità per riga = padova_listing_identity_key(civico_n, lat, lng):
--     • 'C:' || civico_n            se civico presente
--     • 'G:' || round(lat*2500) || ':' || round(lng*2500)   (~40 m grid)
--                                    se civico assente ma coordinate valide
--     • NULL                        altrimenti → riga ESCLUSA dal grouping
--   Le righe con identity_key NULL non partecipano ai gruppi; se un potenziale
--   gruppo perde annunci al punto di non avere più ≥ 2 agenzie distinte,
--   viene semplicemente non-emesso (fail-closed).
--   L'identity_key entra nella chiave di gruppo PRIMA di:
--     • conteggio agenzie
--     • min/max prezzo, ribasso
--     • confidenza
--     • ordinamento / limite / risposta API
--
-- Interfaccia:
--   • Schema di `padova_contendibili` invariato.
--   • View server-only `padova_contendibili_by_zone_v` invariata (filtro
--     MLS + soglia prezzo 8% restano attivi in cascata).
--   • Isolamento zona (resolver quartiere-only) invariato.
--   • ACL invariati.
--   • Dati sorgenti (`padova_listings`) mai mutati.
--
-- Idempotente. Include un DO block di test in-transaction: se una delle
-- 4 asserzioni fallisce la migration abortisce.

BEGIN;

-- 1) Helper immutable riusabile in recompute e nei test. ---------------
...
-- 2) Recompute: identity gate PRIMA di tutto il resto. -----------------
...
BEGIN
  ...
  -- Candidati arricchiti con civico normalizzato e identity_key.
  ...
  -- Identity gate: fail-closed su righe senza civico e senza coordinate.
...
-- 3) Test in-transaction sull'helper identity_key. ---------------------
--    Abortisce la migration se una qualsiasi asserzione fallisce.
DO $test$
DECLARE
  k_fus_a text;  -- Fusinato senza civico, coord A
  k_sav_a text;  -- Savonarola senza civico, stessa coord A (via_n diverso a monte)
  k_civ_5 text;
  k_civ_12 text;
  k_same_a text;
  k_same_b text;
  k_ambig text;
BEGIN
  -- Test 1: senza civico due coord distanti >100 m ⇒ celle diverse.
  ...
  IF k_fus_a = k_sav_a THEN
    RAISE EXCEPTION 'FAIL test 1: Fusinato/Savonarola stessa cella (%=%%)', k_fus_a, k_sav_a;
  END IF;

  -- Test 2: stessa via, civici diversi ⇒ chiavi diverse.
  ...
  IF k_civ_5 = k_civ_12 THEN
    RAISE EXCEPTION 'FAIL test 2: civici diversi con stessa identity_key (% vs %)', k_civ_5, k_civ_12;
  END IF;
  IF k_civ_5 NOT LIKE 'C:%' OR k_civ_12 NOT LIKE 'C:%' THEN
    RAISE EXCEPTION 'FAIL test 2: civico non usato come identity (% / %)', k_civ_5, k_civ_12;
  END IF;

  -- Test 3: stesso civico ⇒ chiave uguale (prezzo non entra nell'identità).
  ...
  IF k_same_a <> k_same_b THEN
    RAISE EXCEPTION 'FAIL test 3: stesso civico produce identity diverse (% vs %)', k_same_a, k_same_b;
  END IF;

  -- Test 4: nessun civico e nessuna coord valida ⇒ NULL (fail-closed).
  k_ambig := public.padova_listing_identity_key(NULL, NULL, NULL);
  IF k_ambig IS NOT NULL THEN
    RAISE EXCEPTION 'FAIL test 4: indirizzo ambiguo NON escluso (%)', k_ambig;
  END IF;
  k_ambig := public.padova_listing_identity_key('', NULL, NULL);
  IF k_ambig IS NOT NULL THEN
    RAISE EXCEPTION 'FAIL test 4b: civico vuoto e coord assenti NON escluso';
  END IF;
