-- ============================================================================
-- Central Core — INTERVENTO 1: FRESHNESS FAIL-CLOSED
-- ----------------------------------------------------------------------------
-- Scopo:
--   A) Rendere temporaneamente fail-closed public.expire_padova_agency_listings:
--      la funzione non esegue alcun UPDATE e ritorna expired=0 con
--      skipped_reason='provider_coverage_not_proven', in attesa di una prova
--      di copertura completa per fonte (idealista/casa/immobiliare).
--   B) Correggere public.promote_padova_collect_v2_to_listings nel solo ramo
--      Idealista: sostituire ON CONFLICT DO NOTHING con DO UPDATE per
--      rinfrescare last_seen_at ed azzerare expired_at SOLO quando la riga
--      staging corrente (selezionata da p_since e conforme ai medesimi
--      controlli di validità già presenti) contiene realmente l'URL.
--      I campi vengono aggiornati con COALESCE(EXCLUDED, existing) per non
--      cancellare informazioni preesistenti.
--
-- Divieti applicati:
--   - Nessun backfill.
--   - Nessun UPDATE diretto delle righe storiche.
--   - Nessuna riattivazione massiva degli expired.
--   - Nessuna modifica alla soglia dei 14 giorni.
--   - Nessun nuovo stato/enum inventato.
--   - Firme, permessi, SECURITY DEFINER, search_path preservati.
--   - Ramo non-Idealista invariato.
-- ============================================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- A) expire_padova_agency_listings — fail-closed
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.expire_padova_agency_listings(p_seen_since timestamp with time zone)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
BEGIN
  -- FAIL-CLOSED: nessun UPDATE finché la copertura provider non è dimostrata
  -- per ciascuna fonte (idealista/casa/immobiliare). Vedi Intervento 1.
  RETURN jsonb_build_object(
    'ok', true,
    'expired', 0,
    'seen_since', p_seen_since,
    'skipped_reason', 'provider_coverage_not_proven'
  );
END
$function$;

-- ---------------------------------------------------------------------------
-- B) promote_padova_collect_v2_to_listings — ramo Idealista con DO UPDATE
-- ---------------------------------------------------------------------------
...
BEGIN
  -- Branch 1: non-idealista portals → UPSERT with field merging (INVARIATO)
  WITH src AS (
  ...
  -- Branch 2: idealista → INSERT ... ON CONFLICT DO UPDATE
  -- Il refresh di last_seen_at ed il reset di expired_at avvengono SOLO se
  -- l'URL è realmente presente in staging (updated_at >= p_since) e supera
  -- gli stessi controlli di validità già usati per l'INSERT.
  -- Nessun refresh basato sul mero trascorrere del tempo.
  WITH src_id AS (
    SELECT DISTINCT ON (portal, url)
      portal, url, raw_address, agency, agency_phone,
      prezzo, mq, locali, bagni, lat, lng,
