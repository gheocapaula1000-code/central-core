...
-- 3) Certificatore contendibili: dedup canonico prima dei conteggi -----------
...
BEGIN
  ...
  -- canonical-listing-dedup-v1: identità canonica dell'annuncio PRIMA di
  -- qualunque conteggio di annunci/agenzie. Lo stesso annuncio canonico
  -- (stesso ID di portale, URL con/senza slash, query o www) contribuisce
  -- una sola riga: quella attiva più recente (last_seen_at, poi id).
  ...
  -- QA post-pubblicazione: nessun gruppo con meno di 2 identità canoniche.
  SELECT count(*) INTO v_bad
    FROM public.padova_contendibili pc
   WHERE (
