-- ═══════════════════════════════════════════════════════════════════
-- Civiko One / Padova — correzione P0 del matcher contendibili v4.
-- Additiva, idempotente, isolata: nessun altro dominio, nessun cron,
-- nessun provider. Patch deterministica del corpo di
-- public.recompute_padova_listings_contendibili(): ogni sostituzione
-- fallisce con eccezione se l'ancora non esiste (fail-closed).
--
-- 1) QA fotografica: _img_grp/_img_ok/_img_cert non esponevano has_asta
--    e has_mls usati dalla QA -> column error certo a runtime. I flag
--    vengono propagati esplicitamente dal candidato al gruppo.
-- 2) Banda 10-15%: prova forte in OR — (>=2 foto pHash condivise)
--    OPPURE (geo <= 30 m + identità unità compatibile + testo forte).
-- 3) Via/civico nel percorso fotografico sono soltanto segnali positivi:
--    una divergenza non è più veto. Zona e comune restano vincolanti.
-- 4) match_version esplicita v4.
-- ═══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- (C) via non è veto nel percorso fotografico + banda 10-15% con OR
  IF position('-- matcher v4: prova forte in OR' in v_src) = 0 THEN
    v_out := replace(v_src,
      ...
      || E'       )\n'
      || E'     );',
      -- Percorso fotografico: la via è soltanto un segnale positivo, mai
      -- un veto. Restano vincolanti zona, comune, struttura e prezzo.
      E'     -- matcher v4: prova forte in OR nella banda 10-15%.\n'
      || E'     AND (\n'
      || E'       greatest(x.prezzo, y.prezzo)::numeric <= least(x.prezzo, y.prezzo)::numeric * 1.10\n'
      || E'       OR (\n'
      || E'         greatest(x.prezzo, y.prezzo)::numeric <= least(x.prezzo, y.prezzo)::numeric * 1.15\n'
