...
|--------|-------|---------|
| `X-Core-Version` | `3.4.6` | Core version that generated the response |
...
| `/scan/photo-wow` | POST | Photo + GPS official report (OMI/ISTAT/OSM Overpass POIs, Nominatim fallback) | ✅ Active |
...
**photoWow / live PWA:** Sottra PWA `getPhotoWow` posts to Core `core-proxy` with `endpoint: /civiko-property-from-photo`. That alias is forwarded to `/sottra/scan/photo-wow` with `x-source-app: sottra`. The report includes official OMI, ISTAT, Nominatim address, and OSM named POIs (`poiEnrichment` / `elencoServiziRilevati`, radius 800 m). Overpass is primary; if it returns no named elements or is down, Nominatim search is the fail-closed fallback (real `name` + computable distance only). Unnamed OSM nodes are dropped. If both fail → `sourceType: unavailable`, no invented names. `core-proxy` also forwards `/sottra/scan/poi-enrichment` and `/sottra/forecast/neighborhood`. Civiko One continues to call `civiko-property-from-photo` directly.
